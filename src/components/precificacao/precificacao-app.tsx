...
export function PrecificacaoApp() {
  ...
  const resultado = useMemo(() => {
    if (custoDireto <= 0 && horas <= 0) return null;
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <FormField label="Custo direto (material/insumos)" htmlFor="custo-direto" required>
              <Input
                id="custo-direto"
                inputMode="numeric"
                placeholder="R$ 0,00"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <FormField label="Quanto quer ganhar por hora" htmlFor="valor-hora">
                <Input
                  id="valor-hora"
                  inputMode="numeric"
                  placeholder="R$ 0,00"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField label="Custos fixos mensais" htmlFor="custos-fixos" hint="Aluguel, internet, ferramentas, etc.">
                <Input
                  id="custos-fixos"
                  inputMode="numeric"
                  placeholder="R$ 0,00"
              ...
              <FormField label="Vendas estimadas por mês" htmlFor="vendas-mensais">
                <Input
                  id="vendas-mensais"
                  type="number"
                  min={1}
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <SliderField label="Taxa do cartão (%)" value={taxaCartao} onChange={setTaxaCartao} max={15} step={0.1} />
              <SliderField label="Imposto (%)" value={imposto} onChange={setImposto} max={30} step={0.5} />
              <SliderField label="Margem de lucro desejada (%)" value={margem} onChange={setMargem} max={80} step={1} />
            </div>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <h2 className="rj-display mb-3 text-base font-bold text-slate-900">Resultado</h2>
            {!resultado ? (
              <p className="text-sm font-medium text-slate-500">Informe ao menos o custo direto ou as horas trabalhadas.</p>
            ) : (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between rounded-xl bg-slate-900 px-4 py-3 text-white">
                  ...
                  <span className="rj-display text-lg font-bold">{formatCurrency(resultado.precoFinal)}</span>
                </div>

                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-xl border border-slate-200 p-3 text-center">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Lucro líquido/venda</p>
                    <p className="rj-display mt-1 text-lg font-bold text-emerald-700">
                      {formatCurrency(resultado.lucroLiquidoPorVenda)}
                    </p>
                    <p className="text-xs text-slate-500">{resultado.margemLiquidaReal.toFixed(1)}% de margem real</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 p-3 text-center">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Markup</p>
                    <p className="rj-display mt-1 text-lg font-bold text-slate-900">{resultado.markup.toFixed(2)}x</p>
                    <p className="text-xs text-slate-500">sobre o custo direto</p>
                  </div>
                </div>

                <p className="text-xs leading-5 text-slate-500">
                  Estimativa educativa — ajuste conforme concorrência, elasticidade de preço e posicionamento da sua
                  marca.
                </p>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className={cn('h-2 w-full cursor-pointer appearance-none rounded-full bg-slate-200 accent-sky-600')}
      />
    </div>
  );
}
