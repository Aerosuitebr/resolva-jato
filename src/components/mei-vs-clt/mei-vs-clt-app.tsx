...
import { AlertTriangle, Copy, MessageCircle, Scale } from 'lucide-react';
...
const FAIXAS: FaixaMei[] = ['comercio-industria', 'servicos', 'comercio-e-servicos'];
...
export function MeiVsCltApp() {
  ...
  const resultadoClt = useMemo(() => {
    ...
    return simularClt({ salarioBruto: salarioClt, dependentes, valeTransporte: vt, planoSaude: plano });
  }, [salarioClt, dependentes, vt, plano]);

  const resultadoMei = useMemo(() => {
    if (faturamento <= 0) return null;
    return simularMei({ faturamentoMensal: faturamento, faixa, custosMensais: custos });
  }, [faturamento, faixa, custos]);

  const diferenca =
    resultadoClt && resultadoMei ? resultadoMei.lucroLiquido - resultadoClt.liquidoMensalEquivalente : null;
  ...
  function resumoTexto() {
    ...
    return [
      ...
      diferenca !== null
        ...
        : '',
      resultadoMei.ultrapassaLimite ? '⚠ Faturamento projetado ultrapassa o limite anual do MEI (R$ 81.000).' : '',
      '',
  ...
  return (
    <AuthGate title="Simulador MEI vs CLT" description="Cadastre-se gratuitamente para comparar os dois cenários.">
      <div className="space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <h2 className="rj-display text-base font-bold text-slate-900">Cenário CLT</h2>
            <FormField label="Salário bruto mensal" htmlFor="salario-clt" required>
              <Input
                id="salario-clt"
                inputMode="numeric"
                placeholder="R$ 0,00"
                value={salarioCltInput}
                onChange={(e) => setSalarioCltInput(formatCurrencyInput(e.target.value))}
              />
            </FormField>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <FormField label="Vale-transporte (mensal)" htmlFor="vt">
                <Input
                  ...
                  placeholder="R$ 0,00"
                  value={vtInput}
                  onChange={(e) => setVtInput(formatCurrencyInput(e.target.value))}
                />
              </FormField>
            </div>
            <FormField label="Plano de saúde (desconto mensal)" htmlFor="plano">
              <Input
                ...
                placeholder="R$ 0,00"
                value={planoInput}
                onChange={(e) => setPlanoInput(formatCurrencyInput(e.target.value))}
              />
            </FormField>
            ...
            {resultadoClt ? (
              <div className="space-y-2 rounded-xl bg-slate-50 p-3 text-sm">
                ...
                <Row label="FGTS depositado (não é líquido, fica retido)" value={resultadoClt.fgtsMensal} muted />
                ...
                <div className="flex items-center justify-between font-bold text-emerald-700">
                  ...
                  <span>{formatCurrency(resultadoClt.liquidoMensalEquivalente)}</span>
                </div>
              </div>
            ) : null}
          </div>
          ...
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <h2 className="rj-display text-base font-bold text-slate-900">Cenário MEI</h2>
            <FormField label="Faturamento mensal estimado" htmlFor="faturamento" required>
              <Input
                id="faturamento"
                inputMode="numeric"
                placeholder="R$ 0,00"
                value={faturamentoInput}
                onChange={(e) => setFaturamentoInput(formatCurrencyInput(e.target.value))}
              />
            </FormField>
            <FormField label="Atividade" htmlFor="faixa">
              <Select id="faixa" value={faixa} onChange={(e) => setFaixa(e.target.value as FaixaMei)}>
                {FAIXAS.map((f) => (
            ...
            >
              <Input
                ...
                placeholder="R$ 0,00"
                value={custosInput}
                onChange={(e) => setCustosInput(formatCurrencyInput(e.target.value))}
              />
            </FormField>

            {resultadoMei ? (
              <div className="space-y-2 rounded-xl bg-slate-50 p-3 text-sm">
                <Row label="DAS (imposto mensal MEI)" value={resultadoMei.das} />
                <Row label="Custos do negócio" value={resultadoMei.custosMensais} />
                ...
                {resultadoMei.ultrapassaLimite ? (
                  <div className="flex items-start gap-2 rounded-lg bg-amber-50 p-2.5 text-xs font-semibold text-amber-900">
                    ...
                    Faturamento anual projetado ultrapassa o limite do MEI (R$ 81.000/ano). Considere migrar para
                    ME/Simples Nacional.
                  </div>
                ) : null}
              </div>
        ...
        {resultadoClt && resultadoMei ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <h2 className="rj-display mb-3 text-base font-bold text-slate-900">Comparativo</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 p-3 text-center">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">CLT (médio/mês)</p>
                <p className="rj-display mt-1 text-xl font-bold text-slate-900">
                  {formatCurrency(resultadoClt.liquidoMensalEquivalente)}
                </p>
              </div>
              <div className="rounded-xl border border-slate-200 p-3 text-center">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">MEI (lucro/mês)</p>
                <p className="rj-display mt-1 text-xl font-bold text-slate-900">
            ...
            {diferenca !== null ? (
              <p
                className={cn(
                  ...
                  diferenca >= 0 ? 'bg-emerald-50 text-emerald-800' : 'bg-sky-50 text-sky-800'
                )}
              >
                {diferenca >= 0
            ...
            <p className="mt-3 text-xs leading-5 text-slate-500">
              Simulação educativa: não considera FGTS acumulado, estabilidade, benefícios, 13º/férias do MEI (não
              existem) nem custos de abertura/fechamento. Consulte um contador antes de decidir.
            </p>
...
function Row({ label, value, muted }: { label: string; value: number; muted?: boolean }) {
  return (
    <div className={cn('flex items-center justify-between', muted && 'text-slate-500')}>
