...
/** Faixa curta: diferencia busca ilimitada de usos das ferramentas / Premium. */
export function TopEnvBanner({ visible = true }: TopEnvBannerProps) {
  ...
  const premium =
    ready && isAuthenticated && usage.unlimited
      ...
      : 'Busca gratuita e ilimitada · 5 usos de ferramentas no plano grátis';

  return (
    <div
      className={`fixed inset-x-0 top-0 z-[200] h-8 overflow-hidden ${
        ready && isAuthenticated && usage.unlimited ? 'bg-emerald-950' : 'bg-slate-900'
      }`}
    >
